...
def DriveUnit_1(val):
	...
	ser_drive_unit_1.write(enc_query) #Motor 1 of encoder 1
	rpm1 = ser_drive_unit_1.read_all() #get left motor (m1) of encoder 1
	...
	if len(rpm1) > 1: 
		...
		try:
			rpm1 = int(rpm1)
			enc1_rpms.data = [rpm1]
			#print("ENC1 Left:", enc1_rpms.data)
...
def command_motors():
	...
	global pub
	pub = rospy.Publisher('left_RPMs', Int32MultiArray, queue_size=10)
